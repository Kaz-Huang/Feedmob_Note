...
const EmbedComponent = ({ node, updateAttributes }: any) => {
  ...
  return (
    <NodeViewWrapper className="my-4 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-900">
      <div className="flex items-center justify-between px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-xs text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-1.5 font-medium">
          {type === 'figma' && <Layout className="w-3.5 h-3.5 text-purple-500" />}
          {type === 'video' && <Play className="w-3.5 h-3.5 text-red-500" />}
          {type === 'bookmark' && <Globe className="w-3.5 h-3.5 text-blue-500" />}
      ...
      {isEditing ? (
        <div className="p-3 bg-white dark:bg-slate-950 flex flex-col gap-2">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 text-xs px-3 py-1.5 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 outline-none"
            />
            <button
              type="button"
              onClick={handleSave}
        ...
        <div className="p-2 bg-white dark:bg-slate-950">
          {type === 'figma' || type === 'video' ? (
            <div className="w-full h-80 rounded overflow-hidden border border-slate-200 dark:border-slate-800">
              <iframe
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 transition group"
            >
              <div className="flex items-center gap-2 overflow-hidden">
                <Globe className="w-4 h-4 text-blue-500 shrink-0" />
                <div className="truncate">
                  <div className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate group-hover:text-emerald-600">
                    {url}
                  </div>
                  <div className="text-[10px] text-slate-400">点击在外部浏览器中打开链接</div>
                </div>
              </div>
